...
from morer.linkage_problem_analysis import graph_construction

from morer.data_io.entity import Entity
from morer.data_io.test_data import famer_constant
import networkx as nx
import csv


def read_data(input_folder: str):
    file_names = os.listdir(input_folder)
    file_names = sorted(file_names, reverse=True)
    if 'json' in file_names[0]:
        return read_json_famer_data(input_folder)
    else:
        return read_csv_famer_data(input_folder)
...
def read_json_famer_data(input_folder: str):
    '''
...
def transform_to_data_sources(entities: dict[str:Entity]) -> tuple[dict[str, dict[str:list]], dict[str, dict[str:int]]]:
    data_sources = {}
    att_indices_dict = {}
    for id, e in entities.items():
        if e.resource not in data_sources:
            data_sources[e.resource] = {}
            att_indices_dict[e.resource] = {}
        data_source = data_sources[e.resource]
        att_indices_source = att_indices_dict[e.resource]
        att_index = 0
        values = ['' for i in range(len(e.properties))]
        for att, value in e.properties.items():
            if att not in att_indices_source:
                att_indices_source[att] = att_index
...
def read_csv_famer_data(input_folder: str):
    '''
        ...
        for row in meta_reader:
            if row[0] == 'v':
                ...
                for att_index, att in enumerate(attribute_list):
                    ...
                    data_type = att.split(':')[1]
                    att_dict[att_index] = (att_name, data_type)
                ...
                if source_pair:
                    if row[2] in source_pair:
                        ...
                        for index, value in enumerate(att_values):
                            ...
                            new_value = new_value.replace("\\:", ':')
                            if att_index_dict[index][1] == 'int' and value.isdigit():
                                new_value = int(value)
                            elif (att_index_dict[index][1] == 'float' or \
                                    att_index_dict[index][1] == 'double') and \
                                    value.isdigit():
                                new_value = float(value)
                            props[att_index_dict[index][0]] = new_value
                    ...
                    for index, value in enumerate(att_values):
                        ...
                        new_value = new_value.replace("\\,", ',')
                        new_value = new_value.replace("\\:", ':')
                        if att_index_dict[index][1] == 'int' and len(value) > 0:
                            new_value = int(value)
                        elif (att_index_dict[index][1] == 'float' or \
                              att_index_dict[index][1] == 'double') and len(value) > 0:
                            new_value = float(value)
                        props[att_index_dict[index][0]] = new_value
                    props[famer_constant.REC_ID] = props[famer_constant.DEXTER_REC_ID]
                    entity.properties = props
