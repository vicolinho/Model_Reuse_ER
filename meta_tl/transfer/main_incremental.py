...
from meta_tl.transfer.incremental.cluster_selection import determine_best_cluster, determine_similar_clusters, \
...
parser.add_argument('--batch_size', '-b', type=int, default=5,
                    help='batch size')
parser.add_argument('--use_sim_quantile', '-sim_q', type=bool, default=False,
                    help='use similarity quantile to decide if new training data are required')
...
USE_SIM_QUANTILE = args.use_sim_quantile
        ...
        if LP_SELECTION_STRATEGY == 'max' or IS_RECLUSTER:
            if len(selected_tasks_list) > 0:
                ...
                if IS_RECLUSTER:
                    ...
                    #print("unsolved lps: {}".format(unsolved_lps_count))
                    ...
                    print("{} - {} ratio of links from solved lps: {}".format(selected_task, unsolved_problem,
                                                                            covered_ratio))
                    combined = (USE_SIM_QUANTILE and covered_ratio > UNSOLVED_RATIO != -1
                             and old_quartile_avg > new_quartile_avg)
                    only_covered_ratio = (covered_ratio > UNSOLVED_RATIO != -1)
                    only_quantile_sim = (UNSOLVED_RATIO != -1 and USE_SIM_QUANTILE and
                                                                  old_quartile_avg > new_quartile_avg)
                    print("cases combined: {} only covered ratio: {} only quantile sim: {}".format(combined,
                                                                                                   only_covered_ratio,
                                                                                                   only_quantile_sim))
                    if ((covered_ratio > UNSOLVED_RATIO != -1) or (UNSOLVED_RATIO != -1 and USE_SIM_QUANTILE and
                                                                  old_quartile_avg > new_quartile_avg) or
                            (USE_SIM_QUANTILE and covered_ratio > UNSOLVED_RATIO != -1
                             and old_quartile_avg > new_quartile_avg)):
                        print("Retrain")
                        ...
                        #print("old features {} ratio compared to Budget {}".format(ratio_training_data,
                        #                                                           old_train_features.shape[0]))
                        ...
                        #print("used budget for retraining {}".format(allocated_budget[selected_task]))
